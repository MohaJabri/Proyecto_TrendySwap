...
export const signup = (first_name, last_name, email, password, re_password, is_professional) => async dispatch => {
  dispatch({
...
export const activate = (uid, token) => async dispatch => {
  ...
  try {
      const res = await axios.post(`${backend_url}/auth/users/activation/`, body, config);
...
export const refresh = () => async dispatch => {
  if (localStorage.getItem('refresh')) {
      ...
      try {
          const res = await axios.post(`${backend_url}/auth/jwt/refresh/`, body, config);
...
export const reset_password = (email) => async dispatch => {
  ...
  try{
      const res = await axios.post(`${backend_url}/auth/users/reset_password/`, body, config);
      ...
      if (res.status === 204) {
          ...
          dispatch(setAlert('Se ha enviado un correo para restablecer tu contraseña', 'green'));
      } else {
          dispatch({
...
export const reset_password_confirm = (uid, token, new_password, re_new_password) => async dispatch => {
  ...
  if (new_password !== re_new_password) {
      ...
      try {
          const res = await axios.post(`${backend_url}/auth/users/reset_password_confirm/`, body, config);
